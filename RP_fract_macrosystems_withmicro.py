...
path_txt_input_folder='/Users/laurenneel/Desktop/Grass_RGB_DSM_Oct22/RP2/'
finput = open(path_txt_input_folder + "RP2_elev_1m", "r")
ELEV = []
ELEV = read_it()
...
finput = open(path_txt_input_folder + "RP2_slope_1m", "r")
SLOPE = []
SLOPE= read_it()
...
finput = open(path_txt_input_folder + "RP2_VARI_1m", "r")
VEG = []
VEG= read_it()
...
finput = open(path_txt_input_folder + "RP2_aspect_1m", "r")
ASPECT = []
ASPECT= read_it()
...
micro_df = pd.read_csv("/Users/laurenneel/Desktop/Grass_RGB_DSM_Oct22/microclim_input_era5/hourly_df_RP_2021.csv")
...
def do_sim():
	...
	class T_e():
		def __init__(self, emissivity = 0.95, phi = 34., longitude = 109., elev = 0., h = 0.07, d = 0.2, emmiss_animal = 0.95, sky = 0.7, r_soil = 0.3, emmiss_soil = 0.94, abs_animal_long = 0.97, abs_animal_short = 0.7, Ta_min0 = 20., Ta_min1 = 20., Ta_min2 = 20., Ta_max0 = 35., Ta_max1 = 35., Ta_max2 = 35., ampl = 15., wind = 0.1):
			...
			self.x_max, self.y_max = 160., 160. #x_max = ncols, y_max = nrows #### CHANGED FROM SEARS 99. for each 8feb!
		...
		def t_e(self, t, J, Tg_shade, Tg_sun, Ta_2m, Ta_shade, Ta_sun, sun, a_s, alpha_s,veg): #UPDATE FOR MICROCLIM INPUT DATA .... t_ground should be replaced with Ta at 2cm... need to set it up for different sun/shade 2cm input
			...
			self.zenith(t,J)
			if 0.<self.Z<=90.:
				if sun ==1 and veg < 0.4: #was or not and
					sun = 1
				else:
					sun = 0.375
				if veg > 0.4:
...
#           #output.write(str(e_range) + "\t" + str(e_fract) + "\t" + str(v_range) + "\t" + str(v_fract) +"\t" + str(t)+ "\t" + str(lizard.position['x']) + "\t" +str(lizard.position['y']) + "\t" + str(lizard.te) + "\t" +str(lizard.tb) + "\t" + str(lizard.active) +"\t" + str(lizard.mei())+"\t" + str(lizard.smr()) + "\t" + str(lizard.net())  + "\t"+ str(lizard.moved)+ "\t"+ str(lizard.ate)+"\n")
	...
	with open('RP_output.csv', mode='w', newline='') as output_file: ## ALL DATA
		fieldnames = ['time', 'J', 'x', 'y', 'te', 'min_te', 'max_te', 'tb', 'total_activity', 'mei', 'net', 'moved', 'smr', 'energy_balance']
	...
	with open('RP_overall_averages.csv', mode='w', newline='') as output_file: 
    ...
    # calculate overall averages
		...
		with open('RP_daily_averages.csv', mode='a', newline='') as output_file:
