...
df_climateData = pd.read_csv("/Users/laurenneel/Desktop/Grass_RGB_DSM_Oct22/microclim_input_era5/hourly_df_LR_2021.csv")
# Create list of splines to predict a temperature for anytime of day
ta_sun_splines, ta_shade_splines, ta_2m_splines, tg_sun_splines, tg_shade_splines= [], [], [], [], []
for day in range(1, 366):
    hours = list(df_climateData[df_climateData['doy'] == day]['hour'])
    temps = list(df_climateData[df_climateData['doy'] == day]['Ta_sun']) # Simply substitute to correct column header for different temperatures
    if day < 365:
        hours.append(25)
        Ta_sun_temps.append(list(df_climateData[df_climateData['doy'] == day + 1]['Ta_sun'])[0])
        Ta_shade_temps.append(list(df_climateData[df_climateData['doy'] == day + 1]['Ta_shade'])[0])
        Ta_2m_temps.append(list(df_climateData[df_climateData['doy'] == day + 1]['Ta_2m'])[0])
        Tg_sun_temps.append(list(df_climateData[df_climateData['doy'] == day + 1]['Tg_sun'])[0])
        Tg_shade_temps.append(list(df_climateData[df_climateData['doy'] == day + 1]['Tg_shade'])[0])
    ta_sun_splines.append(PchipInterpolator(hours, Ta_sun_temps))
    ta_shade_splines.append(PchipInterpolator(hours, Ta_shade_temps))
    ta_2m_splines.append(PchipInterpolator(hours, Ta_2m_temps))
    tg_sun_splines.append(PchipInterpolator(hours, Tg_sun_temps))
    tg_shade_splines.append(PchipInterpolator(hours, Tg_shade_temps))
